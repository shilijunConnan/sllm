...
__global__ void matrixSoftmaxGpuKernel(T *V, T* out, int W, int H) {
    ...
    for(int stride = 0; stride < W; stride += blockDim.x) {
        if (tid + stride < W) {
            ...
            tsum[tid] = tsum[tid] * exp(old - tmax[tid]) + exp(V[memoryStart + stride] - tmax[tid]);
        }
        __syncthreads();
    }
    ...
    for(int stride = blockDim.x >> 1; stride > 0; stride >>= 1) {
        if(tid < stride) {
            ...
            tsum[tid] = tsum[tid] * exp(old - tmax[tid]) + tsum[tid + stride] * exp(tmax[tid + stride] - tmax[tid]);
        }
        __syncthreads();
    }
    // 2. 计算softmax
    for(int stride = 0; stride < W; stride += blockDim.x) {
        if (tid + stride < W) {
            out[memoryStart + stride] = exp(V[memoryStart + stride] - tmax[0]) / tsum[0];
        }
        __syncthreads();
    }
